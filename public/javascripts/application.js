...
socket.on("showPost", function (media) {
  var html =
    "<article class='post'><div class='view loaded'><img class='media' src='/uploads/" +
    media +
    "'" +
    " alt='Nueva Imagen' width='100%' height='100%'></div></article>";
...
socket.on("showInChannel", function (media) {
  var html =
    ...
    media +
    " ' " +
    " alt='Nueva Imagen' width='70%' height='70%'>";
  document.getElementById("newSendPost").innerHTML = html;
});
...
function popupwindow(url, title, w, h) {
  ...
  return window.open(
    ...
    "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=" +
      w +
      ", heigh=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=" +
      w +
      ", height=" +
      h +
      ", top=" +
...
function postsLoaded() {
  ...
  $(".post-tag").bind("keypress", function (event) {
    var regex = new RegExp(
      "[A-Z0-9a-záéíóúàèìòùäëïöüÿâêîôûçæœãõñÁÉÍÓÚÀÈÌÒÙÄËÏÖÜŸÂÊÎÔÛÇÆŒÃÕÑß]+"
    );
...
function checkScroll(target) {
  if (
    $(target).scrollTop() * 1.5 >= $(target).prop("scrollHeight") &&
    !gettingPosts
  ) {
    app.router();
  }
...
var app = new Vue({
  ...
  data: {
    user: localStorage.getItem("user")
      ...
      : null,
    route: null,
    posts: localStorage.getItem("token")
      ? createEmptyPosts(0)
      : createEmptyPosts(3),
    newPost: {
      shareOnFb: false,
  ...
  methods: {
    ...
    up: function () {
      setTimeout(function () {
        $("main, html, body").stop().animate(
          {
            // Se necesita html en vez de o con body en escritorio
            scrollTop: 0,
          },
          300
        );
    ...
    sendNewPost: function (event) {
      if (confirm("¿Desear enviar el post?")) {
        ...
        if (!sendingPost) {
          sendingPost = true;
          this.$http
            .post(domain + "/post/new", this.newPost, {
    ...
    sharePost: function (post) {
      this.$http
        .post(domain + "/post/shareActionOnFb", post, {
          headers: { Authorization: "Bearer " + token },
        })
        .then(
          function (data) {},
    ...
    deletePost: function (post) {
      if (post.edittable) {
        if (confirm("Seguro que desas borrar este post?")) {
          this.$http
            .post(domain + "/post/deletePost", post, {
              headers: { Authorization: "Bearer " + token },
            })
            .then(
              function (data) {},
    ...
    updateTag: function (post) {
      ...
      if (post.newTag != "" && post.newTag != undefined) {
        if (bool) {
          ...
          this.$http
            .post(domain + "/post/updateTag", post, {
              headers: { Authorization: "Bearer " + token },
            })
            .then(
              function () {},
    ...
    changeLanguage: function (language) {
      ...
      this.$http
        .post(domain + "/user/changeLanguage", object, {
          headers: { Authorization: "Bearer " + token },
        })
        .then(
          function () {},
    ...
    suggest: function (word) {
      ...
      var pathRegExp = new RegExp(
        /(^@[0-9A-Za-záéíóúàèìòùäëïöüÿâêîôûçæœãõñÁÉÍÓÚÀÈÌÒÙÄËÏÖÜŸÂÊÎÔÛÇÆŒÃÕÑß%]{1,})$/g
      );
      ...
      if (bool) {
        if (app.lastWordSuggested != word) {
          ...
          simplifyName(word, function (tag) {
            app.$http.get("/list/search/q/" + tag).then(
              function (res) {
    ...
    reportPost: function (post) {
      if (!post.edittable) {
        if (confirm("Seguro que desas reportar este post?")) {
          this.$http
            .post(domain + "/post/reportPost", post, {
              headers: { Authorization: "Bearer " + token },
            })
            .then(
              function () {},
    ...
    setGeo: function () {
      getGeo(
        function (coords) {
          app.$http
            .post(domain + "/user/setGeo", coords, {
              headers: { Authorization: "Bearer " + token },
            })
            .then(
              function () {},
...
function query(route, next, error) {
  ...
  app.$http
    .get(domain + "/list/" + [route, queryStart].join("/"), {
      headers: { Authorization: "Bearer " + token },
    })
    .then(
      function (res) {
