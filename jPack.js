...
jPack.showPosts = function(currenUserId, accessToken, post, next) {
	...
	User.findById(currenUserId, 'savedPosts', function (err, user) {
		...
		for(var i=0; i<post.length; i++) {
			...
			posts[i].media = 'https://juaku-dev.cloudapp.net:5000/uploads/' + post[i].media; // '/uploads/' + post[i].media;
			posts[i].location = {};
	...
	function getUserInfo(id, i) {
		User.findById(id, 'providerId hexCode simpleFirstName firstName simpleLastName lastName profilePic', function (err, user) {
